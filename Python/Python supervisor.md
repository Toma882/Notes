...
supervisor 仅支持 Python2.7，因此守护需要 Python 的地方，需要注意是否更改为 Python3


```
# 勿用
# pip intall supervisor
# 用
yum install supervisor
```

## supervisord 配置
安装完 supervisor 之后，可以运行`echo_supervisord_conf `命令输出默认的配置项，也可以重定向到一个配置文件里：
...
```
[program:userlogin]
command=/usr/local/bin/python3 /usr/local/bin/gunicorn -c /home/wwwroot/sgss_login/gunicorn/gunicorn.conf app:app
directory=/home/wwwroot/sgss_login/
autostart=true
autorestart = true   ; 程序异常退出后自动重启
startretries = 3     ; 启动失败自动重试次数，默认是 3
redirect_stderr = true  ; 把 stderr 重定向到 stdout，默认 false
...
stdout_logfile=/home/wwwroot/sgss_login/gunicorn/gunicorn_supervisor_stdout.log
```
...
supervisorctl -c supervisor.conf start [all]|[x]
supervisorctl -c supervisor.conf stop [all]|[x]
...
## 注意事项
* supervisor适用于前台程序，也就是背后如果有daemon这样的，要设置为daemon=False
*

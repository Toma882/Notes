...
请注意输出的结果，`task 0，1，2，3`是立刻执行的，而task 4要等待前面某个task完成后才执行，这是因为Pool的默认大小在我的电脑上是4，因此，最多同时执行4个进程。这是Pool有意设计的限制，并不是操作系统的限制。如果改成：
`p = Pool(5)`，就可以同时跑5个进程。
...
def change_it(n):
    # 先存后取，结果应该为0:
    # global语句用来声明 balance 是全局变量
    # 当我们在函数内给 balance 赋值时，它的改变映射到我们在主块中使用的 balance 的值
    global balance
    balance = balance + n
    balance = balance - n
...
无论多少线程，同一时刻最多只有一个线程持有该锁，所以，不会造成修改的冲突。创建一个锁就是通过`threading.Lock()`来实现：
...
Queue对象存储在哪？注意到task_worker.py中根本没有创建Queue的代码，所以，Queue对象存储在`task_master.py`进程中：
![分布式进程](assets/分布式进程.png)

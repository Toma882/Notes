...
### 纹理 Texture
游戏领域基本上都用的是 位图，还有程序化生成的纹理 Procedural Texture。

### 贴图 Map/Mapping
把纹理 Texture 通过 UV 坐标映射到3D 物体表面。贴图包含了除了纹理以外其他很多信息，比方说 UV 坐标、贴图输入输出控制等等。
...
在顶点着色器程序输出顶点坐标之后，各个顶点被按照绘制命令中的图元类型参数，以及顶点索引数组被组装成一个个图元（Primitive）。

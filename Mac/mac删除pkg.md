mac osx上大多数应用程序都是通过`.dmg`或者`.pkg`来安装的（当然`brew`方式安装的除外）。

如果是通过`.dmg`方式安装的软件，要删除它就挺简单，直接从osx的应用程序文件夹上删除即可。

那么`.pkg`方式安装的软件又该怎么删除呢？
...
我的机子是osx 10.8.4，pkg历史安装列表在`/private/var/db/receipts`目录下：
